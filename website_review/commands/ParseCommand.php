...
class ParseCommand extends CConsoleCommand {
	private $domain, $ip, $score, $html, $errorcode, $document, $favicon, $content, $image, $links,
		$metatags, $seoanalyse, $validation, $tagcloud, $matrix, $misc;

	private function _init($domain, $ip) {
		$this -> domain = $domain;
		$this -> ip = $ip;
	}

	public function actionUpdate($domain, $ip, $wid) {
		$this -> _init($domain, $ip);
	...
	public function actionInsert($domain, $ip) {
		$this -> _init($domain, $ip);
		if(!$this -> grabHtml()) {
			return $this -> errorcode;
		}
