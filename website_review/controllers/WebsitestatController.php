...
class WebsitestatController extends Controller {
	...
	public function init() {
		...
		if(!$this->website = $this->command
			-> select("id, domain, modified, score")
			-> from("{{website}}")
	...
	public function actionGenerateHTML($domain) {
		...
		$this -> title = Yii::t("meta", "Webstat page title", array('{Domain}' => $this->domain));
		$description=Yii::t("meta", "Webstat page description", array('{Domain}' => $this->domain));
		$cs=Yii::app() -> clientScript;
		$cs -> registerMetaTag(Yii::t("meta", "Webstat page keywords", array('{Domain}' => $this->domain)), "keywords");
	...
	public function actionGeneratePDF($domain) {
        ...
        $this->createPdfFromHtml($html, $pdfFile, $filename);
	}
    ...
    protected function createPdfFromHtml($html, $pdfFile, $filename) {
        ...
        $pdf -> SetTitle(Yii::t("app", "Analyse of {Domain}", array("{Domain}" => $this -> website['domain'])));
        $pdf -> SetSubject(Yii::t("app", "Analyse of {Domain}", array("{Domain}" => $this -> website['domain'])));
